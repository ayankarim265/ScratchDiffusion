...
# Add numpy scalar to safe globals before loading the model
add_safe_globals([np.core.multiarray.scalar])
torch.serialization.add_safe_globals([ModelCheckpoint])  # Allow Lightning classes

def load_from_standard_weights(input_file: str, device: str) -> dict[str, torch.Tensor]:
    try:
        # Try loading with weights_only first (PyTorch 2.6+)
        checkpoint = torch.load(input_file, map_location=device, weights_only=True)
